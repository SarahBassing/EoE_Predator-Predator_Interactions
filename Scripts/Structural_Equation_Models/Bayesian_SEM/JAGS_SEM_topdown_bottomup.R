  #'  --------------------------------------------------------
  #'  JAGS model: top-down, bottom-up, interference SEM
  #'  
  #'  Model description: 
  ...
  #'    e: number of betas for 1-yr lag elk effect (nElk, 1:3, see w for details)
  #'    m: number of betas for 1-yr lag moose effect (nMoose, 1:2, see w for details)
  #'    d: number of betas for 1-yr lag white-tailed deer effect (nDeer, 1:2, see w for details)
  #'    h: number of betas for 1-yr lag harvest effect (nharvest, 1:5, one per species except coy)
  ...
  #'  --------------------------------------------------------
  cat(file = './Outputs/SEM/JAGS_out/JAGS_SEM_topdown_bottomup.txt', "
      model{
      ...
      for(i in 1:nCluster) {
        ...
        mu.wolf.t[i] <- beta.int[2] + beta.wolf[1] * wolf.tmin1[i] + beta.elk[2] * elk.tmin1[i] + beta.moose[2] * moose.tmin1[i] + beta.harvest[1] * wolfHarv.tmin1[i] 
        
        wolf.tmin1[i] ~ dnorm(mu.wolf.tmin1[i], tau.spp.tmin1[1])
        mu.wolf.tmin1[i] <- beta.int.tmin1[2]
        
        lion.t[i] ~ dnorm(mu.lion.t[i], tau.spp[2])  
        mu.lion.t[i] <- beta.int[1] + beta.lion[1] * lion.tmin1[i] + beta.wolf[2] * wolf.tmin1[i] + beta.elk[3] * elk.tmin1[i] + beta.wtd[2] * wtd.tmin1[i] #+ beta.harvest[2] * lionHarv.tmin1[i] 
        
        lion.tmin1[i] ~ dnorm(mu.lion.tmin1[i], tau.spp.tmin1[2])
        mu.lion.tmin1[i] <- beta.int.tmin1[1] 
        
        bear.t[i] ~ dnorm(mu.bear.t[i], tau.spp[3]) 
        mu.bear.t[i] <- beta.int[3] + beta.bear[1] * bear.tmin1[i] + beta.wolf[3] * wolf.tmin1[i] + beta.forest[1] * forest.tmin1[i] # + beta.harvest[3] * bearHarv.tmin1[i]
        
        bear.tmin1[i] ~ dnorm(mu.bear.tmin1[i], tau.spp.tmin1[3]) 
        mu.bear.tmin1[i] <- beta.int.tmin1[3] 

        coy.t[i] ~ dnorm(mu.coy.t[i], tau.spp[4])
        mu.coy.t[i] <- beta.int[4] + beta.coy[1] * coy.tmin1[i] + beta.wolf[4] * wolf.tmin1[i] + beta.lion[2] * lion.tmin1[i] + beta.bear[2] * bear.tmin1[i] 

        coy.tmin1[i] ~ dnorm(mu.coy.tmin1[i], tau.spp.tmin1[4])
        mu.coy.tmin1[i] <- beta.int.tmin1[4] 
        
        elk.t[i] ~ dnorm(mu.elk.t[i], tau.spp[5])
        mu.elk.t[i] <- beta.int[5] + beta.elk[1] * elk.tmin1[i] + beta.wolf[5] * wolf.tmin1[i] + beta.lion[3] * lion.tmin1[i] + beta.forest[2] * forest.tmin1[i] + beta.harvest[4] * elkHarv.tmin1[i] #beta.wsi[3] * wsi.tmin1[i] + 

        elk.tmin1[i] ~ dnorm(mu.elk.tmin1[i], tau.spp.tmin1[5])
        mu.elk.tmin1[i] <- beta.int.tmin1[5]
        
        moose.t[i] ~ dnorm(mu.moose.t[i], tau.spp[6])
        mu.moose.t[i] <- beta.int[6] + beta.moose[1] * moose.tmin1[i] + beta.wolf[6] * wolf.tmin1[i] + beta.forest[3] * forest.tmin1[i] #+ beta.wsi[3] * wsi.tmin1[i]

        moose.tmin1[i] ~ dnorm(mu.moose.tmin1[i], tau.spp.tmin1[6])
        mu.moose.tmin1[i] <- beta.int.tmin1[6]
        
        wtd.t[i] ~ dnorm(mu.wtd.t[i], tau.spp[7])
        mu.wtd.t[i] <- beta.int[7] + beta.wtd[1] * wtd.tmin1[i] + beta.lion[4] * lion.tmin1[i] + beta.forest[4] * forest.tmin1[i] + beta.harvest[5] * deerHarv.tmin1[i] # + beta.wolf[7] * wolf.tmin1[i] + beta.wsi[3] * wsi.tmin1[i]
