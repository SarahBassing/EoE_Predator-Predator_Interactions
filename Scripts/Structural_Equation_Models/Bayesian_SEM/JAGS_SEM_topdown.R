  ...
  #'  --------------------------------------------------------
  cat(file = './Outputs/SEM/JAGS_out/JAGS_SEM_topdown.txt', "
      model{
      ...
      for(i in 1:nSites) {
        ...
        for(y in 2:nYear) {
          ...
          mu.bear[i,y] <- beta.int[3] + beta.bear[1] * bear.latent[i,y-1] + beta.harvest[3] * bearHarv[i,y-1] 
  ...
  #'   mu.lion.t[i] <- beta.int[1] + beta.lion[1] * lion.tmin1[i] + beta.harvest[2] * lionHarv.tmin1[i] #+ cluster.randeff[2,i]
  #'   
  #'   lion.tmin1[i] ~ dnorm(mu.lion.tmin1[i], tau.spp.tmin1[2])
  #'   mu.lion.tmin1[i] <- beta.int.tmin1[1] 
  #'   
  #'   bear.t[i] ~ dnorm(mu.bear.t[i], tau.spp[3]) 
  #'   mu.bear.t[i] <- beta.int[3] + beta.bear[1] * bear.tmin1[i] + beta.harvest[3] * bearHarv.tmin1[i] #+ cluster.randeff[3,i]
  #'   
  #'   bear.tmin1[i] ~ dnorm(mu.bear.tmin1[i], tau.spp.tmin1[3]) 
  #'   mu.bear.tmin1[i] <- beta.int.tmin1[3] 
  #'   
  #'   coy.t[i] ~ dnorm(mu.coy.t[i], tau.spp[4])
  #'   mu.coy.t[i] <- beta.int[4] + beta.coy[1] * coy.tmin1[i] #+ cluster.randeff[4,i]
  #'   
