  #'  --------------------------------------------------------
  #'  JAGS model: bottom-up, interference SEM - REDUCED - with d-Sep test results
  #'  
  #'  Model description: 
  ...
  #'    Results of directed-separation tests are included here, where parameters
  #'    found to not be conditionally independent were included in this version of
  #'    the SEM and parameters found to not have support were removed after those
  #'    identified by d-Sep were added.
  #'  
  #'  Parameters:
  ...
  #'  --------------------------------------------------------
  cat(file = './Outputs/SEM/JAGS_out/JAGS_SEM_bottomup_inter_reduced_w_dSep1.txt', "
      model{
      ...
      #'   sigma.spp.tmin1[k] ~ dnorm(0, 1 / (pow(scale, 2) * aux.tmin1[k])) T(0,)
      ...
      for(i in 1:nCluster) {
        ...
        wolf.tmin1[i] ~ dnorm(mu.wolf.tmin1[i], tau.spp[1])
        ...
        lion.tmin1[i] ~ dnorm(mu.lion.tmin1[i], tau.spp[2])
        ...
        bear.tmin1[i] ~ dnorm(mu.bear.tmin1[i], tau.spp[3]) 
        ...
        coy.tmin1[i] ~ dnorm(mu.coy.tmin1[i], tau.spp[4])
        mu.coy.tmin1[i] <- beta.int.tmin1[4] 
        
        elk.t[i] ~ dnorm(mu.elk.t[i], tau.spp[5])
        mu.elk.t[i] <- beta.int[5] + beta.elk[1] * elk.tmin1[i] + beta.forest[1] * forest.tmin1[i] + beta.wsi[1] * wsi.tmin1[i] 

        elk.tmin1[i] ~ dnorm(mu.elk.tmin1[i], tau.spp[5])
        mu.elk.tmin1[i] <- beta.int.tmin1[5]
        
        moose.t[i] ~ dnorm(mu.moose.t[i], tau.spp[6])
        mu.moose.t[i] <- beta.int[6] + beta.moose[1] * moose.tmin1[i] + beta.forest[2] * forest.tmin1[i] + beta.wsi[2] * wsi.tmin1[i]

        moose.tmin1[i] ~ dnorm(mu.moose.tmin1[i], tau.spp[6])
        mu.moose.tmin1[i] <- beta.int.tmin1[6]
        
        wtd.t[i] ~ dnorm(mu.wtd.t[i], tau.spp[7])
        mu.wtd.t[i] <- beta.int[7] + beta.wtd[1] * wtd.tmin1[i] + beta.forest[3] * forest.tmin1[i] + beta.wsi[3] * wsi.tmin1[i] + beta.bear[2] * bear.t[i] + beta.coy[2] * coy.tmin1[i] 
        
        wtd.tmin1[i] ~ dnorm(mu.wtd.tmin1[i], tau.spp[7])
  ...
  #'   mu.wtd.tmin1[i] <- beta.int.tmin1[7] 
        ...
        # wolf.t[i] ~ dnorm(mu.wolf.t[i], tau.spp[1])
        # mu.wolf.t[i] <- beta.int[2] + beta.wolf[1] * wolf.tmin1[i] + beta.elk[2] * elk.tmin1[i] + beta.elk[5] * elk.t[i]
        # # beta.moose[2] * moose.tmin1[i] + beta.elk[2] * elk.tmin1[i] + beta.wsi[4] * wsi.tmin1[i] + beta.forest[5] * forest.tmin1[i] + beta.bear[2] * bear.tmin1[i]  
        # 
        # wolf.tmin1[i] ~ dnorm(mu.wolf.tmin1[i], tau.spp.tmin1[1])
        # mu.wolf.tmin1[i] <- beta.int.tmin1[2]
        # 
        # lion.t[i] ~ dnorm(mu.lion.t[i], tau.spp[2])  
        # mu.lion.t[i] <- beta.int[1] + beta.lion[1] * lion.tmin1[i] + beta.wolf[5] * wolf.t[i] #+ beta.coy[2] * coy.t[i] 
        # # + beta.wolf[2] * wolf.tmin1[i] beta.wtd[2] * wtd.tmin1[i] + beta.bear[4] * bear.t[i] + beta.wsi[5] * wsi.tmin1[i] + beta.forest[6] * forest.tmin1[i] + beta.moose[3] * moose.t[i] + beta.wtd[4] * wtd.t[i] 
        # 
        # lion.tmin1[i] ~ dnorm(mu.lion.tmin1[i], tau.spp.tmin1[2])
        # mu.lion.tmin1[i] <- beta.int.tmin1[1] 
        # 
        # bear.t[i] ~ dnorm(mu.bear.t[i], tau.spp[3]) 
        # mu.bear.t[i] <- beta.int[3] + beta.bear[1] * bear.tmin1[i] 
        # # + beta.wolf[3] * wolf.tmin1[i] + beta.forest[4] * forest.tmin1[i] + beta.elk[3] * elk.tmin1[i] + beta.wsi[6] * wsi.tmin1[i] + beta.moose[3] * moose.tmin1[i] + beta.moose[4] * moose.t[i] + beta.elk[6] * elk.t[i]
        # 
        # bear.tmin1[i] ~ dnorm(mu.bear.tmin1[i], tau.spp.tmin1[3]) 
        # mu.bear.tmin1[i] <- beta.int.tmin1[3] 
        # 
        # coy.t[i] ~ dnorm(mu.coy.t[i], tau.spp[4])
        # mu.coy.t[i] <- beta.int[4] + beta.coy[1] * coy.tmin1[i] + beta.wtd[3] * wtd.tmin1[i] + beta.wtd[5] * wtd.t[i]  
        # # + beta.wolf[4] * wolf.tmin1[i] + beta.forest[7] * forest.tmin1[i] + beta.moose[5] * moose.t[i]  + beta.elk[7] * elk.t[i] + beta.bear[5] * bear.t[i] + beta.wolf[6] * wolf.t[i] + beta.wsi[7] * wsi.tmin1[i] + 
        # 
        # coy.tmin1[i] ~ dnorm(mu.coy.tmin1[i], tau.spp.tmin1[4])
        # mu.coy.tmin1[i] <- beta.int.tmin1[4] 
        # 
        # elk.t[i] ~ dnorm(mu.elk.t[i], tau.spp[5])
        # mu.elk.t[i] <- beta.int[5] + beta.elk[1] * elk.tmin1[i] + beta.bear[3] * bear.tmin1[i] #+ beta.moose[6] * moose.t[i]
        # # + beta.forest[1] * forest.tmin1[i] + beta.wsi[1] * wsi.tmin1[i]  
        # 
        # elk.tmin1[i] ~ dnorm(mu.elk.tmin1[i], tau.spp.tmin1[5])
        # mu.elk.tmin1[i] <- beta.int.tmin1[5]
        # 
        # moose.t[i] ~ dnorm(mu.moose.t[i], tau.spp[6])
        # mu.moose.t[i] <- beta.int[6] + beta.moose[1] * moose.tmin1[i] 
        # # + beta.forest[2] * forest.tmin1[i] + beta.wsi[2] * wsi.tmin1[i]
        # 
        # moose.tmin1[i] ~ dnorm(mu.moose.tmin1[i], tau.spp.tmin1[6])
        # mu.moose.tmin1[i] <- beta.int.tmin1[6]
        # 
        # wtd.t[i] ~ dnorm(mu.wtd.t[i], tau.spp[7])
        # mu.wtd.t[i] <- beta.int[7] + beta.wtd[1] * wtd.tmin1[i] + beta.forest[3] * forest.tmin1[i] + beta.bear[4] * bear.tmin1[i] + beta.bear[6] * bear.t[i]  
        # #  + beta.wsi[3] * wsi.tmin1[i] + beta.moose[4] * moose.tmin1[i] + beta.moose[7] * moose.t[i] + beta.wolf[7] * wolf.t[i] + beta.elk[4] * elk.tmin1[i]
        # 
