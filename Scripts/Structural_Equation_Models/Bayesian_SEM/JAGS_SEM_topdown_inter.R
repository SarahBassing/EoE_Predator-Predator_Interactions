...
cat(file = './Outputs/SEM/JAGS_out/JAGS_SEM_topdown_inter.txt', "
      ...
      for(i in 1:nCluster) {
        ...
        mu.wolf.t[i] <- wolf.t_1[1] * wolf.tmin1[i] + harvest.t_1[1] * harvest.tmin1[i]  
        
        lion.t[i] ~ dnorm(mu.lion.t[i], tau.spp[2])
        mu.lion.t[i] <- lion.t_1[1] * lion.tmin1[i] + wolf.t_1[2] * wolf.tmin1[i] + bear.t_1[2] * bear.tmin1[i]

        bear.t[i] ~ dnorm(mu.bear.t[i], tau.spp[3])
        mu.bear.t[i] <- bear.t_1[1] * bear.tmin1[i] + wolf.t_1[3] * wolf.tmin1[i]

        coy.t[i] ~ dnorm(mu.coy.t[i], tau.spp[4])
        mu.coy.t[i] <- coy.t_1[1] * coy.tmin1[i] + wolf.t_1[4] * wolf.tmin1[i] + lion.t_1[2] * lion.tmin1[i]

        elk.t[i] ~ dnorm(mu.elk.t[i], tau.spp[5])
        mu.elk.t[i] <- elk.t_1[1] * elk.tmin1[i] + wolf.t_1[5] * wolf.tmin1[i] + lion.t_1[3] * lion.tmin1[i] + bear.t_1[3] * bear.tmin1[i]

        moose.t[i] ~ dnorm(mu.moose.t[i], tau.spp[6])
        mu.moose.t[i] <- moose.t_1[1] * moose.tmin1[i] + wolf.t_1[6] * wolf.tmin1[i]

        wtd.t[i] ~ dnorm(mu.wtd.t[i], tau.spp[7])
        mu.wtd.t[i] <- wtd.t_1[1] * wtd.tmin1[i] + wolf.t_1[7] * wolf.tmin1[i] + lion.t_1[4] * lion.tmin1[i] + bear.t_1[4] * bear.tmin1[i] + coy.t_1[2] * coy.tmin1[i]
