  #'  --------------------------------------------------------
  #'  JAGS model: top-down, interference SEM finalized after d-Sep
  #'  
  #'  Model description: 
  ...
  #'    h: number of betas for 1-yr lag harvest effect (nharvest, one per predator except coy)
  ...
  #'  --------------------------------------------------------
  cat(file = './Outputs/SEM/JAGS_out/JAGS_SEM_topdown_inter_final.txt', "
      model{
      ...
      for(i in 1:nSites) {
        ...
        for(y in 2:nYear) {
          ...
          mu.lion[i,y] <- beta.int[1] + beta.harvest[1] * lionHarv[i,y-1] + beta.wolf[2] * wolf.latent[i,y-1] 
             
          wolf.latent[i,y] ~ dnorm(mu.wolf[i,y], tau.spp[2])
          mu.wolf[i,y] <- beta.int[2] + beta.wolf[1] * wolf.latent[i,y-1] + beta.harvest[2] * wolfHarv[i,y-1] + beta.bear[2] * bear.latent[i,y]
          ...
          mu.coy[i,y] <- beta.int[4] + beta.coy[1] * coy.latent[i,y-1] + beta.wolf[4] * wolf.latent[i,y-1] + beta.lion[1] * lion.latent[i,y-1]

          elk.latent[i,y] ~ dnorm(mu.elk[i,y], tau.spp[5])
          mu.elk[i,y] <- beta.int[5] + beta.elk[1] * elk.latent[i,y-1] + beta.wolf[5] * wolf.latent[i,y-1] + beta.lion[2] * lion.latent[i,y-1] #+ beta.harvest[4] * elkHarv[i,y-1] 
          ...
          mu.wtd[i,y] <- beta.int[7] + beta.wtd[1] * wtd.latent[i,y-1] + beta.lion[3] * lion.latent[i,y-1] + beta.coy[2] * coy.latent[i,y-1] + beta.bear[3] * bear.latent[i,y-1] # + beta.harvest[5] * deerHarv[i,y-1]
      ...
      #'  ------------------------------------
      #'  Interference-mediated where predator.A.tmin2 --> predator.B.tmin1 --> prey.t
      #'  Via mountain lion................................
      indirect.wolf.lion.elk <- beta.wolf[2] * beta.lion[2]      # effect of wolf.tmin2 on lion.tmin1 --> effect of lion.tmin1 on elk.t
      indirect.wolf.lion.wtd <- beta.wolf[2] * beta.lion[3]      # effect of wolf.tmin2 on lion.tmin1 --> effect of lion.tmin1 on wtd.t
      indirect.wolf.lion.coy <- beta.wolf[2] * beta.lion[1]      # effect of wolf.tmin2 on lion.tmin1 --> effect of lion.tmin1 on coy.t
      
      #'  Via black bear...................................
      indirect.wolf.bear.wtd <- beta.wolf[3] * beta.bear[3]      # effect of wolf.tmin2 on bear.tmin1 --> effect of bear.tmin1 on wtd.t
      
      #'  Via coyote.......................................
      indirect.wolf.coy.wtd <- beta.wolf[4] * beta.coy[2]        # effect of wolf.tmin2 on coy.tmin1 --> effect of coy.tmin1 on wtd.t
      indirect.lion.coy.wtd <- beta.lion[1] * beta.coy[2]        # effect of lion.tmin2 on coy.tmin1 --> effect of coy.tmin1 on wtd.t
      ...
      indirect.wolf.coy.self <- beta.wolf[4] * beta.coy[1]       # effect of wolf.tmin2 on coy.tmin1 --> effect of coy.tmin1 on coy.t
      indirect.lion.coy.self <- beta.lion[1] * beta.coy[1]       # effect of lion.tmin2 on coy.tmin1 --> effect of coy.tmin1 on coy.t
      indirect.bear.wolf.self <- beta.bear[2] * beta.wolf[1]     # effect of bear.tmin1 on wolf.tmin1 --> effect of wolf.tmin1 on wolf.t (note the contemporaneous relationship)
      
      #'  Via predator.A.tmin1 --> prey.tmin1 --> prey.t........................
      indirect.wolf.elk.self <- beta.wolf[5] * beta.elk[1]       # effect of wolf.tmin2 on elk.tmin1 --> effect of elk.tmin1 on elk.t
      indirect.lion.elk.self <- beta.lion[2] * beta.elk[1]       # effect of lion.tmin2 on elk.tmin1 --> effect of elk.tmin1 on elk.t
      indirect.wolf.moose.self <- beta.wolf[6] * beta.moose[1]   # effect of wolf.tmin2 on moose.tmin1 --> effect of moose.tmin1 on moose.t
      indirect.lion.wtd.self <- beta.lion[3] * beta.wtd[1]       # effect of lion.tmin2 on wtd.tmin1 --> effect of wtd.tmin1 on wtd.t
      indirect.coy.wtd.self <- beta.coy[2] * beta.wtd[1]         # effect of coy.tmin2 on wtd.tmin1 --> effect of wtd.tmin1 on wtd.t
      indirect.bear.wtd.self <- beta.bear[3] * beta.wtd[1]       # effect of bear.tmin2 on wtd.tmin1 --> effect of wtd.tmin1 on wtd.t
