  #'  --------------------------------------------------------
  #'  JAGS model: top-down SEM finalized after d-Sep
  #'  
  #'  Model description: 
  ...
  #'    l: number of betas for 1-yr lag mountain lion effect (nLion, 1:3, see w for details)
  #'    b: number of betas for 1-yr lag black bear effect (nBear, 1:3, see w for details)
  ...
  cat(file = './Outputs/SEM/JAGS_out/JAGS_SEM_topdown_final.txt', "
      ...
      for(i in 1:nCluster) {
        ...
        mu.wolf.t[i] <- beta.int[2] + beta.wolf[1] * wolf.tmin1[i] + beta.harvest[1] * deerHarv.tmin1[i]         
        
        wolf.tmin1[i] ~ dnorm(mu.wolf.tmin1[i], tau.spp.tmin1[1])
        mu.wolf.tmin1[i] <- beta.int.tmin1[2]
        
        lion.t[i] ~ dnorm(mu.lion.t[i], tau.spp[2])  
        mu.lion.t[i] <- beta.int[1] + beta.lion[1] * lion.tmin1[i] + beta.bear[2] * bear.t[i]        
