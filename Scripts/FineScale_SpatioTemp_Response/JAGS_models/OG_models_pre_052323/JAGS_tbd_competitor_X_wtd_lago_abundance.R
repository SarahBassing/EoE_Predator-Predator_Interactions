  #'  ----------------------------------------
  #'  Latency model - competitor * prey RIA (wtd & lagomorph) effect
  ...
  cat(file = "./Outputs/Time_btwn_Detections/tbd_competitor_X_wtd_lago_abundance.txt", "
      model{
      ...
      for(i in 1:ntbd){
        ...
        log(tbd_mu[i]) <- alpha0 + beta.competitor[covs[i,2]] + beta.prey[1]*covs[i,8] + beta.prey[2]*covs[i,9] + 
                          beta.interaction.wtd[covs[i,2]]*covs[i,8] + beta.interaction.lago[covs[i,2]]*covs[i,9] + alpha[site[i]]
      }
