  ...
  #'  the species of competitor and the relative abundance of elk/moose/wtd on TBD. 
  #'  -----------------------------------------
  
  cat(file = "./Outputs/Time_btwn_Detections/tbd_competitor_X_elk_moose_wtd_abundance_noRE.txt", "
      model{
      ...
      for(comp in 1:4) {
        spp.tbd[comp] <- exp(alpha0 + beta.competitor[comp] + beta.prey[1]*0 + beta.prey[2]*0 + beta.prey[3]*0 + 
                             beta.interaction.elk[comp]*0 + beta.interaction.moose[comp]*0 + beta.interaction.wtd[comp]*0)
      }
      ...
      for(i in 1:100){
        for(comp in 1:4){
          spp.tbd.elk[i,comp] <- exp(alpha0 + beta.competitor[comp] + beta.prey[1]*newcovs[i,1] + beta.prey[2]*0 + beta.prey[3]*0 + 
                                     beta.interaction.elk[comp]*newcovs[i,1] + beta.interaction.moose[comp]*0 + beta.interaction.wtd[comp]*0)
        }
      }
      ...
      for(i in 1:100){
        for(comp in 1:4){
          spp.tbd.moose[i,comp] <- exp(alpha0 + beta.competitor[comp] + beta.prey[1]*0 + beta.prey[2]*newcovs[i,2] + beta.prey[3]*0 + 
                                     beta.interaction.elk[comp]*0 + beta.interaction.moose[comp]*newcovs[i,2] + beta.interaction.wtd[comp]*0)
        }
      }
      ...
      for(i in 1:100){
        for(comp in 1:4){
          spp.tbd.wtd[i,comp] <- exp(alpha0 + beta.competitor[comp] + beta.prey[1]*0 + beta.prey[2]*0 + beta.prey[3]*newcovs[i,3] + 
                                     beta.interaction.elk[comp]*0 + beta.interaction.moose[comp]*0 + beta.interaction.wtd[comp]*newcovs[i,3])
        }
      }
