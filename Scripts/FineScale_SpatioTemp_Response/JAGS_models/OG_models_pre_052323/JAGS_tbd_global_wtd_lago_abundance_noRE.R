  ...
  cat(file = "./Outputs/Time_btwn_Detections/tbd_global_wtd_lago_abundance_noRE.txt", "
      ...
      for(i in 1:ntbd){
        ...
        log(tbd_mu[i]) <- alpha0 + beta.competitor[covs[i,2]] + beta.prey[1]*covs[i,8] + beta.prey[2]*covs[i,9] + beta.div*covs[i,10] +
                          beta.interaction.wtd[covs[i,2]]*covs[i,8] + beta.interaction.lago[covs[i,2]]*covs[i,9] + beta.interaction[covs[i,2]]*covs[i,10]
      }
      ...
      for(comp in 1:4) {
        spp.tbd[comp] <- exp(alpha0 + beta.competitor[comp] + beta.prey[1]*0 + beta.prey[2]*0 + beta.div*0 +
                             beta.interaction.wtd[comp]*0 + beta.interaction.lago[comp]*0 + beta.interaction[comp]*0)
      }
      
      #'  Mean TBD per competitor across range of wtd relative abundance values
      for(i in 1:100){
        for(comp in 1:4){
          spp.tbd.wtd[i,comp] <- exp(alpha0 + beta.competitor[comp] + beta.prey[1]*newcovs[i,3] + beta.prey[2]*0 + beta.div*0 +
                                     beta.interaction.wtd[comp]*newcovs[i,3] + beta.interaction.lago[comp]*0 + beta.interaction[comp]*0)
        }
      }
      
      #'  Mean TBD per competitor across range of lagomorph relative abundance values 
      for(i in 1:100){
        for(comp in 1:4){
          spp.tbd.lago[i,comp] <- exp(alpha0 + beta.competitor[comp] + beta.prey[1]*0 + beta.prey[2]*newcovs[i,4] + beta.div*0 +
                                      beta.interaction.wtd[comp]*0 + beta.interaction.lago[comp]*newcovs[i,4] + beta.interaction[comp]*0)
        }
      }
      ...
      for(i in 1:100){
        for(comp in 1:4){
          spp.tbd.div[i,comp] <- exp(alpha0 + beta.competitor[comp] + beta.prey[1]*0 + beta.prey[2]*0 + beta.div*newcovs[i,5] +
                                     beta.interaction.wtd[comp]*0 + beta.interaction.lago[comp]*0 + beta.interaction[comp]*newcovs[i,5])
        }
      }
