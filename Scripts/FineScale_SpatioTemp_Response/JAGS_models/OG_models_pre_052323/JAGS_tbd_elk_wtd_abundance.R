#'  ----------------------------------------
#'  Latency model - prey RIA (elk & wtd) effect
...
cat(file = "./Outputs/Time_btwn_Detections/tbd_elk_wtd_abundance.txt", "
      model{
      ...
      for(i in 1:ntbd){
        ...
        log(tbd_mu[i]) <- alpha0 + beta.prey[1]*covs[i,5] + beta.prey[2]*covs[i,8] + alpha[site[i]]
      }
