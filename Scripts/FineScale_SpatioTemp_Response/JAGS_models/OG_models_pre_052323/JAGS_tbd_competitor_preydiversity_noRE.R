  #'  ----------------------------------------
  #'  Latency model - competitor + prey diversity effect, no random effect
  ...
  cat(file = "./Outputs/Time_btwn_Detections/tbd_competitor_preydiversity_noRE.txt", "
      model{
      ...
      for(i in 1:ntbd){
        ...
        log(tbd_mu[i]) <- alpha0 + beta.competitor[covs[i,2]] + beta.div*covs[i,10]
      }
      ...
      for(comp in 1:4) {
        spp.tbd[comp] <- exp(alpha0 + beta.competitor[comp] + beta.div*0)
      }
      
      #'  Mean TBD per competitor across range of prey diversity values
      for(i in 1:100){
        for(comp in 1:4){
          spp.tbd.div[i,comp] <- exp(alpha0 + beta.competitor[comp] + beta.div*newcovs[i,5])
        }
      }
