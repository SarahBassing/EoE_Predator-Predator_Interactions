#'  ----------------------------------------
#'  Latency model - competitor x prey diversity effect, no random effect
...
cat(file = "./Outputs/Time_btwn_Detections/tbd_competitor_X_preydiversity_noRE.txt", "
      model{
      ...
      for(i in 1:ntbd){
        ...
        log(tbd_mu[i]) <- alpha0 + beta.competitor[covs[i,2]] + beta.div*covs[i,10] + beta.interaction[covs[i,2]]*covs[i,10]
      }
