  #'  ----------------------------------------
  #'  Latency model - prey RIA (wtd & lagomorphs) effect
  ...
  cat(file = "./Outputs/Time_btwn_Detections/tbd_wtd_lago_abundance.txt", "
      model{
      ...
      for(i in 1:ntbd){
        ...
        log(tbd_mu[i]) <- alpha0 + beta.prey[1]*covs[i,8] + beta.prey[2]*covs[i,9] + alpha[site[i]]
      }
