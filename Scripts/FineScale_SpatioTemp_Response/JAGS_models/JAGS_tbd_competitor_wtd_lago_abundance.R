  #'  ----------------------------------------
  #'  Latency model - competitor + prey RIA (wtd & lagomorphs) effect
  ...
  #'  relative abundance of wtd/lagomorphs influences TBD. 
  #'  -----------------------------------------
  
  cat(file = "./Outputs/Time_btwn_Detections/tbd_competitor_wtd_lago_abundance.txt", "
      model{
      ...
      for(i in 1:ntbd){
        ...
        log(tbd_mu[i]) <- alpha0 + beta.competitor[covs[i,2]] + beta.prey[1]*covs[i,8] + beta.prey[2]*covs[i,9] + alpha[site[i]]
      }
      ...
      #'  Mean TBD per competitor across range of lagomorphs relative abundance values 
      for(i in 1:100){
        for(comp in 1:4){
