  ...
  cat(file = './Outputs/Hilger_RNmodel/RNmodel_JAGS_code_mod5_elk.txt', "
      model{
        ...
        for(i in 1:nsites){
          N[i] ~ dpois(lambda[i])
          lambda[i] <- exp(beta0 + b.year[year[i]] + b.maxHQ*mean_HQ[i] + b.cvHQ*cv_HQ[i] + 
                           b.maxTbio*mean_Tbio[i] + b.cvTbio*cv_Tbio[i] + b.selected*total_selected[i] + 
