  ...
  spp_smr <- list("elk", "moose", "whitetaileddeer")
  ...
  split_DH <- function(dh) {
    ...
    july <- dh[,31:61]
    august <- dh[,62:92]
    month_list <- list(june, july, august)
    return(month_list)
  }
  #'  Monthly list order: Elk[[1]], Moose[[2]], White-tailed Deer[[3]]; June[[1]], July[[2]], August[[3]]
  ...
  #'  e.g., all June elk detections (2020, 2021, 2022) stacked into a single detection history 
  DH_elk_jun <- rbind(DH_eoe20s_monthly[[1]][[1]], DH_eoe21s_monthly[[1]][[1]], DH_eoe22s_monthly[[1]][[1]])
  DH_elk_jul <- rbind(DH_eoe20s_monthly[[1]][[2]], DH_eoe21s_monthly[[1]][[2]], DH_eoe22s_monthly[[1]][[2]])
  DH_elk_aug <- rbind(DH_eoe20s_monthly[[1]][[3]], DH_eoe21s_monthly[[1]][[3]], DH_eoe22s_monthly[[1]][[3]])
  DH_moose_jun <- rbind(DH_eoe20s_monthly[[2]][[1]], DH_eoe21s_monthly[[2]][[1]], DH_eoe22s_monthly[[2]][[1]])
  DH_moose_jul <- rbind(DH_eoe20s_monthly[[2]][[2]], DH_eoe21s_monthly[[2]][[2]], DH_eoe22s_monthly[[2]][[2]])
  DH_moose_aug <- rbind(DH_eoe20s_monthly[[2]][[3]], DH_eoe21s_monthly[[2]][[3]], DH_eoe22s_monthly[[2]][[3]])
  DH_wtd_jun <- rbind(DH_eoe20s_monthly[[3]][[1]], DH_eoe21s_monthly[[3]][[1]], DH_eoe22s_monthly[[3]][[1]])
  DH_wtd_jul <- rbind(DH_eoe20s_monthly[[3]][[2]], DH_eoe21s_monthly[[3]][[2]], DH_eoe22s_monthly[[3]][[2]])
  DH_wtd_aug <- rbind(DH_eoe20s_monthly[[3]][[3]], DH_eoe21s_monthly[[3]][[3]], DH_eoe22s_monthly[[3]][[3]])
  
  #'  List for easier use
  DH_elk_list <- list(DH_elk_jun, DH_elk_jul, DH_elk_aug)
  DH_moose_list <- list(DH_moose_jun, DH_moose_jul, DH_moose_aug)
  DH_wtd_list <- list(DH_wtd_jun, DH_wtd_jul, DH_wtd_aug)
  ...
  stations_eoe20s <- format_covs(sites_20s, dets = DH_eoe20s_RNmod[[1]]) %>%
    dplyr::select(-ID)
  stations_eoe21s <- format_covs(sites_21s, dets = DH_eoe21s_RNmod[[1]]) %>%
    dplyr::select(-ID)
  stations_eoe22s <- format_covs(sites_22s, dets = DH_eoe22s_RNmod[[1]]) %>%
    dplyr::select(-ID)
  ...
  #'  Stacked detection histories (per species and month)
  save(DH_elk_list, file = "./Data/Side_quests/Hilger/DH_elk_RNmod.RData")
  save(DH_moose_list, file = "./Data/Side_quests/Hilger/DH_moose_RNmod.RData")
  ...
  list_names <- c("June_DH", "July_DH", "August_DH")
  data_JAGS_bundle_elk <- lapply(DH_elk_list, bundle_dat, cov = station_stats)
  data_JAGS_bundle_moose <- lapply(DH_moose_list, bundle_dat, cov = station_stats)
  ...
  ninit_moose <- lapply(DH_moose_list, initial_n)
  ninit_wtd <- lapply(DH_wtd_list, initial_n)
  
  #'  Parameters monitored
  params <- c("beta0", "beta1", "beta2", "beta3", "alpha0", "alpha1", # beta4, beta5,
  ...
  source("./Scripts/Side_quests/Hilger_Predator_RNmodel/RNmodel_JAGS_code_mod1.R")
  source("./Scripts/Side_quests/Hilger_Predator_RNmodel/RNmodel_JAGS_code_mod2.R")
  ...
  ######  Model 1  ######
  start.time = Sys.time()
  inits_elk_June <- function(){list(N = ninit_elk[[1]])}
  RN_elk_june_mod1 <- jags(data_JAGS_bundle_elk[[1]], inits = inits_elk_June, params,
                      "./Outputs/Painter_RNmodel/RNmodel_JAGS_code_mod1.txt",
                      n.adapt = na, n.chains = nc, n.thin = nt, n.iter = ni, 
                      n.burnin = nb, parallel = TRUE)
  end.time <- Sys.time(); (run.time <- end.time - start.time)
  print(RN_elk_june_mod1$summary)
  which(RN_elk_june_mod1$summary[,"Rhat"] > 1.1)
  mcmcplot(RN_elk_june_mod1$samples)
  save(RN_elk_june_mod1, file = paste0("./Outputs/Hilger_RNmodel/JAGS_out/RN_elk_june_mod1_", Sys.Date(), ".RData"))
  
  ######  Model 2  ######
  start.time = Sys.time()
  inits_elk_June <- function(){list(N = ninit_elk[[1]])}
  RN_elk_june_mod2 <- jags(data_JAGS_bundle_elk[[1]], inits = inits_elk_June, params,
                      "./Outputs/Painter_RNmodel/RNmodel_JAGS_code_mod2.txt",
                      n.adapt = na, n.chains = nc, n.thin = nt, n.iter = ni, 
                      n.burnin = nb, parallel = TRUE)
  end.time <- Sys.time(); (run.time <- end.time - start.time)
  print(RN_elk_june_mod2$summary)
  which(RN_elk_june_mod2$summary[,"Rhat"] > 1.1)
  mcmcplot(RN_elk_june_mod2$samples)
  save(RN_elk_june_mod2, file = paste0("./Outputs/Hilger_RNmodel/JAGS_out/RN_elk_june_mod2_", Sys.Date(), ".RData"))
  ...
  ######  Model 1  ######
  start.time = Sys.time()
  inits_elk_July <- function(){list(N = ninit_elk[[2]])}
  RN_elk_july_mod1 <- jags(data_JAGS_bundle_elk[[2]], inits = inits_elk_July, params,
                      "./Outputs/Painter_RNmodel/RNmodel_JAGS_code_mod1.txt",
                      n.adapt = na, n.chains = nc, n.thin = nt, n.iter = ni, 
                      n.burnin = nb, parallel = TRUE)
  end.time <- Sys.time(); (run.time <- end.time - start.time)
  ...
  start.time = Sys.time()
  inits_elk_July <- function(){list(N = ninit_elk[[2]])}
  RN_elk_july_mod2 <- jags(data_JAGS_bundle_elk[[2]], inits = inits_elk_July, params,
                      "./Outputs/Painter_RNmodel/RNmodel_JAGS_code_mod2.txt",
                      n.adapt = na, n.chains = nc, n.thin = nt, n.iter = ni, 
                      n.burnin = nb, parallel = TRUE)
  end.time <- Sys.time(); (run.time <- end.time - start.time)
  ...
  ######  Model 1  ######
  start.time = Sys.time()
  inits_elk_Aug <- function(){list(N = ninit_elk[[3]])}
  RN_elk_aug_mod1 <- jags(data_JAGS_bundle_elk[[3]], inits = inits_elk_Aug, params,
                      "./Outputs/Painter_RNmodel/RNmodel_JAGS_code_mod1.txt",
                      n.adapt = na, n.chains = nc, n.thin = nt, n.iter = ni, 
                      n.burnin = nb, parallel = TRUE)
  end.time <- Sys.time(); (run.time <- end.time - start.time)
  ...
  start.time = Sys.time()
  inits_elk_Aug <- function(){list(N = ninit_elk[[3]])}
  RN_elk_aug_mod2 <- jags(data_JAGS_bundle_elk[[3]], inits = inits_elk_Aug, params,
                     "./Outputs/Painter_RNmodel/RNmodel_JAGS_code_mod2.txt",
                     n.adapt = na, n.chains = nc, n.thin = nt, n.iter = ni, 
                     n.burnin = nb, parallel = TRUE)
  end.time <- Sys.time(); (run.time <- end.time - start.time)
  ...
  ######  Model 1  ######
  start.time = Sys.time()
  inits_wtd_June <- function(){list(N = ninit_wtd[[1]])}
  RN_wtd_june_mod1 <- jags(data_JAGS_bundle_wtd[[1]], inits = inits_wtd_June, params,
                           "./Outputs/Painter_RNmodel/RNmodel_JAGS_code_mod1.txt",
                           n.adapt = na, n.chains = nc, n.thin = nt, n.iter = ni, 
                           n.burnin = nb, parallel = TRUE)
  end.time <- Sys.time(); (run.time <- end.time - start.time)
  print(RN_wtd_june_mod1$summary)
  which(RN_wtd_june_mod1$summary[,"Rhat"] > 1.1)
  mcmcplot(RN_wtd_june_mod1$samples)
  save(RN_wtd_june_mod1, file = paste0("./Outputs/Hilger_RNmodel/JAGS_out/RN_wtd_june_mod1_", Sys.Date(), ".RData"))
  
  ######  Model 2  ######
  start.time = Sys.time()
  inits_wtd_June <- function(){list(N = ninit_wtd[[1]])}
  RN_wtd_june_mod2 <- jags(data_JAGS_bundle_wtd[[1]], inits = inits_wtd_June, params,
                           "./Outputs/Painter_RNmodel/RNmodel_JAGS_code_mod2.txt",
                           n.adapt = na, n.chains = nc, n.thin = nt, n.iter = ni, 
                           n.burnin = nb, parallel = TRUE)
  end.time <- Sys.time(); (run.time <- end.time - start.time)
  print(RN_wtd_june_mod2$summary)
  which(RN_wtd_june_mod2$summary[,"Rhat"] > 1.1)
  mcmcplot(RN_wtd_june_mod2$samples)
  save(RN_wtd_june_mod2, file = paste0("./Outputs/Hilger_RNmodel/JAGS_out/RN_wtd_june_mod2_", Sys.Date(), ".RData"))
  ...
  RN_wtd_july_mod1 <- jags(data_JAGS_bundle_wtd[[2]], inits = inits_wtd_July, params,
                           "./Outputs/Painter_RNmodel/RNmodel_JAGS_code_mod1.txt",
                           n.adapt = na, n.chains = nc, n.thin = nt, n.iter = ni, 
                           n.burnin = nb, parallel = TRUE)
  end.time <- Sys.time(); (run.time <- end.time - start.time)
  ...
  start.time = Sys.time()
  inits_wtd_July <- function(){list(N = ninit_wtd[[2]])}
  RN_wtd_july_mod2 <- jags(data_JAGS_bundle_wtd[[2]], inits = inits_wtd_July, params,
                           "./Outputs/Painter_RNmodel/RNmodel_JAGS_code_mod2.txt",
                           n.adapt = na, n.chains = nc, n.thin = nt, n.iter = ni, 
                           n.burnin = nb, parallel = TRUE)
  end.time <- Sys.time(); (run.time <- end.time - start.time)
  ...
  ######  Model 1  ######
  start.time = Sys.time()
  inits_wtd_Aug <- function(){list(N = ninit_wtd[[3]])}
  RN_wtd_aug_mod1 <- jags(data_JAGS_bundle_wtd[[3]], inits = inits_wtd_Aug, params,
                          "./Outputs/Painter_RNmodel/RNmodel_JAGS_code_mod1.txt",
                          n.adapt = na, n.chains = nc, n.thin = nt, n.iter = ni, 
                          n.burnin = nb, parallel = TRUE)
  end.time <- Sys.time(); (run.time <- end.time - start.time)
  ...
  start.time = Sys.time()
  inits_wtd_Aug <- function(){list(N = ninit_wtd[[3]])}
  RN_wtd_aug_mod2 <- jags(data_JAGS_bundle_wtd[[3]], inits = inits_wtd_Aug, params,
                          "./Outputs/Painter_RNmodel/RNmodel_JAGS_code_mod2.txt",
                          n.adapt = na, n.chains = nc, n.thin = nt, n.iter = ni, 
                          n.burnin = nb, parallel = TRUE)
  end.time <- Sys.time(); (run.time <- end.time - start.time)
