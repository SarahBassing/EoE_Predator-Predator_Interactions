  ...
  cat(file = './Outputs/Hilger_RNmodel/RNmodel_JAGS_code_cvTbio.txt', "
      model{
        ...
        #'  Continuous effects for maximum and CV high quality biomass
        ...
        for(i in 1:nsites){
          N[i] ~ dpois(lambda[i])
          lambda[i] <- exp(beta0 + b.year[year[i]] + b.cvTbio*cv_Tbio[i])
