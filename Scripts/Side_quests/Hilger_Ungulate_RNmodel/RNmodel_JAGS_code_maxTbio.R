  ...
  cat(file = './Outputs/Hilger_RNmodel/RNmodel_JAGS_code_maxTbio.txt', "
        model{
        ...
        #'  Continuous effects for max high quality biomass
        ...
        for(i in 1:nsites){
          N[i] ~ dpois(lambda[i])
          lambda[i] <- exp(beta0 + b.year[year[i]] + b.maxTbio*max_Tbio[i])
