  ...
  cat(file = './Outputs/Hilger_RNmodel/RNmodel_JAGS_code_elk_aug_global_2.txt', "
      model{
        ...
        for(i in 1:nsites){
          N[i] ~ dpois(lambda[i])
          lambda[i] <- exp(beta0 + b.year[year[i]] + b.meanHQ*mean_HQ[i] + b.meanTbio*mean_Tbio[i] + 
                           b.cvTbio*cv_Tbio[i] + b.predicted*total_predicted[i] +  
