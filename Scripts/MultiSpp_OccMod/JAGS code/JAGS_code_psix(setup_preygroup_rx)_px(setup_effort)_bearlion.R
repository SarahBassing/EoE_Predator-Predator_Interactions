  #'  ----------------------------------------------------
  #'  Relative abundance of functional prey groups - bear-lion model
  ...
  #'  Excluding livestock effect b/c do not expect bear or lion distributions to 
  #'  be influenced by cattle in late summer
  #'  ----------------------------------------------------
  
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psix(setup_preygroup_rx)_px(setup_effort)_bearlion.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate order: Spp1 & Spp2 = Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + SmallDeer[5] + BigDeer[6]
          ...
          #'  ...for state Spp12
          #'  Covariate order: Spp12 = Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + SmallDeer[5] + BigDeer[6] 
          psiSpp12[i] <- betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,3] + betaSpp12[4]*psi_inxs_cov[i,4] + betaSpp12[5]*psi_inxs_cov[i,5] + betaSpp12[6]*psi_inxs_cov[i,6]
