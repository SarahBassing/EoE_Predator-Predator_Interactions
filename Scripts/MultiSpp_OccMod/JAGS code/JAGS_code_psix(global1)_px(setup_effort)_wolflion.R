  #'  ------------------------------------
  #'  Global model #1 - wolf-lion model
  ...
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psix(global1)_px(setup_effort)_wolflion.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate order: Spp1 & Spp2 = Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + Dist2Burbs[12] + LogNearestRd[13] + Human[14] + SmallDeer[5] + BigDeer[6] + Livestock[15]
          ...
          psiSpp2[i] <- betaSpp2[1]*psi_cov[i,1] + betaSpp2[2]*psi_cov[i,2] + betaSpp2[3]*psi_cov[i,3] + betaSpp2[4]*psi_cov[i,4] + betaSpp2[5]*psi_cov[i,12] + betaSpp2[6]*psi_cov[i,13] + betaSpp2[7]*psi_cov[i,14] + betaSpp2[8]*psi_cov[i,5] + betaSpp2[9]*psi_cov[i,6] + betaSpp2[10]*psi_cov[i,15] + eta[psi_cov[i,16]]
