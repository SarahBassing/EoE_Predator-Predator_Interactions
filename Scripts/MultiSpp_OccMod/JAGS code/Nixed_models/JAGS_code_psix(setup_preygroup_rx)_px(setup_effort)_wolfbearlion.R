  #'  ----------------------------------------------------
  #'  Relative abundance of functional prey groups - wolf-bear, wolf-lion model
  ...
  #'  But excluding livestock effect on bear|no wolf (lion|no wolf) sub-model b/c do 
  #'  not expect bear/lion distribution to be influenced by cattle in the absence of wolves
  #'  ----------------------------------------------------
  
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psix(setup_preygroup_rx)_px(setup_effort)_wolfbearlion.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate order: Spp2 = Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + SmallDeer[5] + BigDeer[6] 
          ...
          psiSpp2[i] <- betaSpp2[1]*psi_cov[i,1] + betaSpp2[2]*psi_cov[i,2] + betaSpp2[3]*psi_cov[i,3] + betaSpp2[4]*psi_cov[i,4] + betaSpp2[5]*psi_cov[i,5] + betaSpp2[6]*psi_cov[i,6] + etaSpp2[psi_cov[i,16]]
            
          #'  ...for state Spp12
          #'  Covariate order: Spp12 = Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + SmallDeer[5] + BigDeer[6] + Livestock[15] 
          psiSpp12[i] <- betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,3] + betaSpp12[4]*psi_inxs_cov[i,4] + betaSpp12[5]*psi_inxs_cov[i,5] + betaSpp12[6]*psi_inxs_cov[i,6] + betaSpp12[7]*psi_inxs_cov[i,15]
