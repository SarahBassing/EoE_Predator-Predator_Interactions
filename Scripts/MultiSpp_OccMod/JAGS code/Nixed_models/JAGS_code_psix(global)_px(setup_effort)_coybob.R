  ...
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psix(global)_px(setup_effort)_coybob.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate order: Spp1 & Spp2 = Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + SppDiversity[6] + White-tailed Deer[10] + Lagomorph[11]
          psiSpp1[i] <- betaSpp1[1]*psi_cov[i,1] + betaSpp1[2]*psi_cov[i,2] + betaSpp1[3]*psi_cov[i,3] + betaSpp1[4]*psi_cov[i,4] + betaSpp1[5]*psi_cov[i,6] + betaSpp1[6]*psi_cov[i,10] + betaSpp1[7]*psi_cov[i,11] + eta[psi_cov[i,16]]
          psiSpp2[i] <- betaSpp2[1]*psi_cov[i,1] + betaSpp2[2]*psi_cov[i,2] + betaSpp2[3]*psi_cov[i,3] + betaSpp2[4]*psi_cov[i,4] + betaSpp2[5]*psi_cov[i,6] + betaSpp2[6]*psi_cov[i,10] + betaSpp2[7]*psi_cov[i,11] + eta[psi_cov[i,16]]
          
          #'  ...for state Spp12
          #'  Covariate order: Spp12 = Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + SppDiversity[6] + White-tailed Deer[10] + Lagomorph[11]
          psiSpp12[i] <- psiSpp1[i] + psiSpp2[i] + betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,3] + betaSpp12[4]*psi_inxs_cov[i,4] + betaSpp12[5]*psi_inxs_cov[i,6] + betaSpp12[6]*psi_cov[i,10] + betaSpp1[7]*psi_cov[i,11]
