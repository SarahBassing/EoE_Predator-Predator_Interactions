  #'  ----------------------------------------------------
  #'  Relative abundance of functional prey groups - wolf-coyote (and lion-bobcat) model
  ...
  #'  Prey species included: small deer (md & wtd), big deer (elk & moose), livestock, & lagomorph
  #'  Excluding livestock effect on coyote|no wolf (or bobcat|no lion) sub-model 
  #'  b/c do not expect mesopredator distribution to be influenced by cattle in 
  ...
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psix(setup_preygroup_rx)_px(setup_effort)_wolfcoy.txt', "
      model{
        ...
        #'  Coyote slopes (lagomorph but no livestock effect)
        for(fo_psi in 2:7){                         # fo occupancy slopes 
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate order: Spp1 = Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + SmallDeer[5] + BigDeer[6] + Livestock[15] 
          #'  Covariate order: Spp2 = Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + SmallDeer[5] + BigDeer[6] + Lagomorph[11] 
          psiSpp1[i] <- betaSpp1[1]*psi_cov[i,1] + betaSpp1[2]*psi_cov[i,2] + betaSpp1[3]*psi_cov[i,3] + betaSpp1[4]*psi_cov[i,4] + betaSpp1[5]*psi_cov[i,5] + betaSpp1[6]*psi_cov[i,6] + betaSpp1[7]*psi_cov[i,15] + etaSpp1[psi_cov[i,16]]
          psiSpp2[i] <- betaSpp2[1]*psi_cov[i,1] + betaSpp2[2]*psi_cov[i,2] + betaSpp2[3]*psi_cov[i,3] + betaSpp2[4]*psi_cov[i,4] + betaSpp2[5]*psi_cov[i,5] + betaSpp2[6]*psi_cov[i,6] + betaSpp2[7]*psi_cov[i,11] + etaSpp2[psi_cov[i,16]]
            
          #'  ...for state Spp12
          #'  Covariate order: Spp12 = Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + SmallDeer[5] + BigDeer[6] + Livestock[15] + Lagomorph[11]  
          psiSpp12[i] <- betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,3] + betaSpp12[4]*psi_inxs_cov[i,4] + betaSpp12[5]*psi_inxs_cov[i,5] + betaSpp12[6]*psi_inxs_cov[i,6] + betaSpp12[7]*psi_inxs_cov[i,15] + betaSpp12[8]*psi_inxs_cov[i,11]
