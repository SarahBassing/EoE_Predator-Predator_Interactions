  #'  ------------------------------------
  #'  Global model #1 - wolf-bear, wolf-lion model
  ...
  #'  Prey species included: small deer (md & wtd), big deer (elk & moose), livestock
  #'  Exclude livestock from bear/lion submodel b/c do not expect cattle to 
  #'  influence distributions in absence of wolves
  #'  ------------------------------------
  
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psix(global1)_px(setup_effort)_wolfbearlion.txt', "
      model{
        ...
        #'  Bear/lion slopes (no livestock effect)
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate order: Spp1 = Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + Dist2Burbs[12] + LogNearestRd[13] + Human[14] + SmallDeer[5] + BigDeer[6] + Livestock[15]
          #'  Covariate order: Spp2 = Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + Dist2Burbs[12] + LogNearestRd[13] + Human[14] + SmallDeer[5] + BigDeer[6]
          psiSpp1[i] <- betaSpp1[1]*psi_cov[i,1] + betaSpp1[2]*psi_cov[i,2] + betaSpp1[3]*psi_cov[i,3] + betaSpp1[4]*psi_cov[i,4] + betaSpp1[5]*psi_cov[i,12] + betaSpp1[6]*psi_cov[i,13] + betaSpp1[7]*psi_cov[i,14] + betaSpp1[8]*psi_cov[i,5] + betaSpp1[9]*psi_cov[i,6] + betaSpp1[10]*psi_cov[i,15] + eta[psi_cov[i,16]]
          psiSpp2[i] <- betaSpp2[1]*psi_cov[i,1] + betaSpp2[2]*psi_cov[i,2] + betaSpp2[3]*psi_cov[i,3] + betaSpp2[4]*psi_cov[i,4] + betaSpp2[5]*psi_cov[i,12] + betaSpp2[6]*psi_cov[i,13] + betaSpp2[7]*psi_cov[i,14] + betaSpp2[8]*psi_cov[i,5] + betaSpp2[9]*psi_cov[i,6] + eta[psi_cov[i,16]]
          
          #'  ...for state Spp12
          #'  Covariate order: Spp12 = Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + Dist2Burbs[12] + LogNearestRd[13] + Human[14] + SmallDeer[5] + BigDeer[6] + Livestock[15] 
          psiSpp12[i] <- betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,3] + betaSpp12[4]*psi_inxs_cov[i,4] + betaSpp12[5]*psi_inxs_cov[i,12] + betaSpp12[6]*psi_inxs_cov[i,13] + betaSpp12[7]*psi_inxs_cov[i,14] + betaSpp12[8]*psi_inxs_cov[i,5] + betaSpp12[9]*psi_inxs_cov[i,6] + betaSpp12[10]*psi_inxs_cov[i,15] 
