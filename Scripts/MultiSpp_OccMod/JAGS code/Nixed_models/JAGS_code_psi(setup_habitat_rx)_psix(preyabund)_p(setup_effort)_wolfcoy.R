  ...
  #'  Excluding elk, livestock & moose effects on coyote model b/c do not expect 
  #'  large ungualtes to influence coyote occurrence but including lagomorph on  
  #'  coyote model b/c important prey item for them.
  #'  ------------------------------------
  
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psi(setup_habitat_rx)_psix(preyabund)_p(setup_effort)_wolfcoy.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate order: Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + Elk[7] + Moose[8] + White-tailed deer[10] + Lagomorph[11] # + Livestock[15]
          ...
          #'  ...for state Spp12
          #'  Covariate order: Spp12 = Intercept[1] + Setup[2] + Elk[7] + Moose[8] + White-tailed deer[10] + Lagomorph[11] # + Livestock[15]
          psiSpp12[i] <- psiSpp1[i] + psiSpp2[i] + betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,7] + betaSpp12[4]*psi_inxs_cov[i,8] + betaSpp12[5]*psi_inxs_cov[i,10] + betaSpp12[6]*psi_inxs_cov[i,11] # + betaSpp12[7]*psi_inxs_cov[i,15] 
