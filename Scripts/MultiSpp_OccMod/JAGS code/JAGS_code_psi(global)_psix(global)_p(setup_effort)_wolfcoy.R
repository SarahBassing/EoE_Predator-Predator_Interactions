  #'  ------------------------------------
  #'  Global wolf-coyote interaction model
  ...
  #'  Prey species included: elk, moose, wtd
  #'  Excluding elk, & moose effects on coyote model b/c do not expect 
  #'  large ungualtes to influence coyote occurrence but including lagomorph on  
  #'  coyote model b/c important prey item for them.
  #'  ------------------------------------
  
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psi(global)_psix(global)_p(setup_effort)_wolfcoy.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate order: Spp1: Intercept[1] + Setup[2] + Year[3] + Forest[4] + Elevation[5] + TRI[17] + Elk[7] + Moose[8] + White-tailed deer[10] + SppDiversity[6]
          #'  Covariate order: Spp2: Intercept[1] + Setup[2] + Year[3] + Forest[4] + Elevation[5] + TRI[17] + White-tailed deer[10] + Lagomorph[11] + SppDiversity[6]
          psiSpp1[i] <- betaSpp1[1]*psi_cov[i,1] + betaSpp1[2]*psi_cov[i,2] + betaSpp1[3]*psi_cov[i,3] + betaSpp1[4]*psi_cov[i,4] + betaSpp1[5]*psi_cov[i,5] + betaSpp1[6]*psi_cov[i,17] + betaSpp1[7]*psi_cov[i,7] + betaSpp1[8]*psi_cov[i,8] + betaSpp1[9]*psi_cov[i,10] + betaSpp1[10]*psi_cov[i,6]
          psiSpp2[i] <- betaSpp2[1]*psi_cov[i,1] + betaSpp2[2]*psi_cov[i,2] + betaSpp2[3]*psi_cov[i,3] + betaSpp2[4]*psi_cov[i,4] + betaSpp2[5]*psi_cov[i,5] + betaSpp2[6]*psi_cov[i,17] + betaSpp2[7]*psi_cov[i,10] + betaSpp2[8]*psi_cov[i,11] + betaSpp2[9]*psi_cov[i,6] 
          
          #'  ...for state Spp12
          #'  Covariate order: Spp12 = Intercept[1] + Setup[2] + Elk[7] + Moose[8] + White-tailed deer[10] + Lagomorph[11] + SppDiversity[6]
          psiSpp12[i] <- psiSpp1[i] + psiSpp2[i] + betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,7] + betaSpp12[4]*psi_inxs_cov[i,8] + betaSpp12[5]*psi_inxs_cov[i,10] + betaSpp12[6]*psi_inxs_cov[i,11] + betaSpp12[7]*psi_inxs_cov[i,6] 
