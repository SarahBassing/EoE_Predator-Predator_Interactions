  ...
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psi(global)_psix(global)_p(setup_effort)_wolfcoy.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate order: Intercept[1] + Setup[2] + Year[5] + Elevation[3] + Forest[4] + Elk[7] + Moose[8] + White-tailed deer[10] + Lagomorph[11] + SppDiversity[6]
          psiSpp1[i] <- betaSpp1[1]*psi_cov[i,1] + betaSpp1[2]*psi_cov[i,2] + betaSpp1[3]*psi_cov[i,5] + betaSpp1[4]*psi_cov[i,3] + betaSpp1[5]*psi_cov[i,4] + betaSpp1[6]*psi_cov[i,7] + betaSpp1[7]*psi_cov[i,8] + betaSpp1[8]*psi_cov[i,10] + betaSpp1[9]*psi_cov[i,6]
