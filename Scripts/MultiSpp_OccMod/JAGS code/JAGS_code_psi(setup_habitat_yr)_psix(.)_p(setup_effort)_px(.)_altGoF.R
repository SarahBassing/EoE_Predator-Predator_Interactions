  ...
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psi(setup_habitat_yr)_psix(.)_p(setup_effort)_px(.)_altGoF.txt', "
      model{
        ...
        for(i in 1:nsites) {
          for(j in 1:nsurveys) {
            ...
            #' y.sim[i,j] ~ dcat(rdm[i, j, (1:ncat), z[i]]) #z.sim[i]
            #' 
