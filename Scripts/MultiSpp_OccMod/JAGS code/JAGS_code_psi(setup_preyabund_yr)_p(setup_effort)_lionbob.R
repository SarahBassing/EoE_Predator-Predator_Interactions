  #'  ------------------------------------
  #'  Prey relative abundance, no species interactions - lion-bobcat model
  ...
  #'  Including lagomorph for bobcats only b/c important prey item but excluding 
  #'  elk b/c don't expect elk distributions to influence bobcat distributions.
  #'  ------------------------------------
  
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psi(setup_preyabund_yr)_p(setup_effort)_lionbob.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate order: Spp1 = Intercept[1] + Setup[2] + Year[3] + Forest[4] + Elevation[5] + TRI[17] + Elk[7] + White-tailed deer[10] 
