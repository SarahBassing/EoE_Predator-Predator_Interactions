  #'  ----------------------------------------------------
  #'  Relative abundance of functional prey groups - lion-bob model
  ...
  #'  Prey species included: small deer (md & wtd), big deer (elk & moose)
  #'  Including lagomorph on bobcat submodels b/c important prey item for them
  #'  Excluding big deer from bobcat b/c don't expect elk/moose to influence distributions
  #'  ----------------------------------------------------
  
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psix(setup_preygroup_rx)_px(setup_effort)_lionbob.txt', "
      model{
        ...
        #'  Lion slopes (big deer, no lagomorph)
        for(fo_psi in 2:6){                         # fo occupancy slopes 
          betaSpp1[fo_psi] ~ dnorm(0, 0.1)
        }
        
        #'  Bobcat slopes (lagomorph, no big deer)
        for(fo_psi in 2:6){                         # fo occupancy slopes 
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate order: Spp1 = Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + SmallDeer[5] + BigDeer[6]
          #'  Covariate order: Spp2 = Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + SmallDeer[5] + Lagomorph[11]
          psiSpp1[i] <- betaSpp1[1]*psi_cov[i,1] + betaSpp1[2]*psi_cov[i,2] + betaSpp1[3]*psi_cov[i,3] + betaSpp1[4]*psi_cov[i,4] + betaSpp1[5]*psi_cov[i,5] + betaSpp1[6]*psi_cov[i,6] + etaSpp1[psi_cov[i,16]]
          psiSpp2[i] <- betaSpp2[1]*psi_cov[i,1] + betaSpp2[2]*psi_cov[i,2] + betaSpp2[3]*psi_cov[i,3] + betaSpp2[4]*psi_cov[i,4] + betaSpp2[5]*psi_cov[i,5] + betaSpp2[6]*psi_cov[i,11] + etaSpp2[psi_cov[i,16]]
            
          #'  ...for state Spp12
          #'  Covariate order: Spp12 = Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + SmallDeer[5] + BigDeer[6] + Lagomorph[11]
          psiSpp12[i] <- betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,3] + betaSpp12[4]*psi_inxs_cov[i,4] + betaSpp12[5]*psi_inxs_cov[i,5] + betaSpp12[6]*psi_inxs_cov[i,6] + betaSpp12[7]*psi_inxs_cov[i,11]
