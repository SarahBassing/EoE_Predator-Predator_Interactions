  ...
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psi(setup_habitat_rx)_psix(preydiversity)_p(setup_effort).txt', "
      model{
        ...
        #' #'  Second order occupancy slopes for categorical dominant prey species 
        #' #'  (1 = elk, 2 = wtd, 3 = other)
        #' betaSpp12_domprey[1] <- 0
        #' for(so_prey in 2:3){
        #'   betaSpp12_domprey[so_prey] ~ dnorm(0, 0.1)
        #' }
