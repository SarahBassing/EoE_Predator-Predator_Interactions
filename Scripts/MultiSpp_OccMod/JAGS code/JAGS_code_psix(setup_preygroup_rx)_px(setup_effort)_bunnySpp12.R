  #'  ----------------------------------------------------
  #'  Relative abundance of prey groups - includes lagomorph effect on all psi
  ...
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psix(setup_preygroup_rx)_px(setup_effort)_bunnySpp12.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate order: Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + DomPrey[5] + SppDiversity[6] + Lagomorph[11] #used to be SmallDeer[5] + BigDeer[6] + Livestock[15]
          psiSpp1[i] <- betaSpp1[1]*psi_cov[i,1] + betaSpp1[2]*psi_cov[i,2] + betaSpp1[3]*psi_cov[i,3] + betaSpp1[4]*psi_cov[i,4] + betaSpp1[5]*psi_cov[i,5] + betaSpp1[6]*psi_cov[i,6] + betaSpp1[7]*psi_cov[i,11] + etaSpp1[psi_cov[i,16]] # betaSpp1[7]*psi_cov[i,15]  + betaSpp1[8]*psi_cov[i,11]
          psiSpp2[i] <- betaSpp2[1]*psi_cov[i,1] + betaSpp2[2]*psi_cov[i,2] + betaSpp2[3]*psi_cov[i,3] + betaSpp2[4]*psi_cov[i,4] + betaSpp2[5]*psi_cov[i,5] + betaSpp2[6]*psi_cov[i,6] + betaSpp2[7]*psi_cov[i,11] + etaSpp2[psi_cov[i,16]] #betaSpp2[7]*psi_cov[i,15] + betaSpp2[8]*psi_cov[i,11]
            
          #'  ...for state Spp12
          psiSpp12[i] <- betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,3] + betaSpp12[4]*psi_inxs_cov[i,4] + betaSpp12[5]*psi_inxs_cov[i,5] + betaSpp12[6]*psi_inxs_cov[i,6] + betaSpp12[7]*psi_inxs_cov[i,11] #betaSpp12[7]*psi_inxs_cov[i,15] + betaSpp12[8]*psi_inxs_cov[i,11]
