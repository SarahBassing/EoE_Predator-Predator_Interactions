  #'  ------------------------------------
  #'  Prey diversity model - wolf-bear (and lion-bear) model
  ...
  #'  sub-model b/c do not expect bear distribution to be influenced by 
  #'  cattle or moose in the absence of apex predators
  #'  ------------------------------------
  
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psix(setup_preydiversity_rx)_px(setup_effort)_wolfbear.txt', "
      model{
        ...
        #'  Bear slopes (no moose or livestock effect)
