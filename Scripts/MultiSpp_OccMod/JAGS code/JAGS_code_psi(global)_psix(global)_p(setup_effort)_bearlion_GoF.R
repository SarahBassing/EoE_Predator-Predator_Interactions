  #'  ------------------------------------
  #'  Global bear-lion interaction model
  ...
  #'  Excluding moose & livestock effects b/c do not expect bear or lion
  #'  distributions to be influenced by cattle/moose in late summer
  #'  ------------------------------------
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psi(global)_psix(global)_p(setup_effort)_bearlion_GoF.txt', "
      model{
    ...
    # Or Kleiven et al. script https://dataverse.no/file.xhtml?persistentId=doi:10.18710/ZLW59W/EY8ZRT&version=4.
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate order: Intercept[1] + Setup[2] + Year[3] + Forest[4] + Elevation[5] + TRI[17] + Elk[7] + White-tailed deer[10] + SppDiversity[6]
          ...
          psiSpp2[i] <- betaSpp2[1]*psi_cov[i,1] + betaSpp2[2]*psi_cov[i,2] + betaSpp2[3]*psi_cov[i,3] + betaSpp2[4]*psi_cov[i,4] + betaSpp2[5]*psi_cov[i,5] + betaSpp2[6]*psi_cov[i,17] + betaSpp2[7]*psi_cov[i,7] + betaSpp2[8]*psi_cov[i,10] + betaSpp2[9]*psi_cov[i,6]

          #'  ...for state Spp12
          #'  Covariate order: Spp12 = Intercept[1] + Setup[2] + Elk[7] + White-tailed deer[10] + SppDiversity[6]
          psiSpp12[i] <- psiSpp1[i] + psiSpp2[i] + betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,7] + betaSpp12[4]*psi_inxs_cov[i,10] + betaSpp12[5]*psi_inxs_cov[i,6]
