  ...
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psi(setup_habitat_yr)_p(setup_effort).txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate order: Intercept[1] + Setup[2] + Year[5] + TRI[17] + Forest[4] + Footprint[16] # + Elevation[3] 
          psiSpp1[i] <- betaSpp1[1]*psi_cov[i,1] + betaSpp1[2]*psi_cov[i,2] + betaSpp1[3]*psi_cov[i,5] + betaSpp1[4]*psi_cov[i,17] + betaSpp1[5]*psi_cov[i,4] + betaSpp1[6]*psi_cov[i,16] # + betaSpp1[4]*psi_cov[i,3]
          psiSpp2[i] <- betaSpp2[1]*psi_cov[i,1] + betaSpp2[2]*psi_cov[i,2] + betaSpp2[3]*psi_cov[i,5] + betaSpp2[4]*psi_cov[i,17] + betaSpp2[5]*psi_cov[i,4] + betaSpp2[6]*psi_cov[i,16] # + betaSpp2[4]*psi_cov[i,3]
