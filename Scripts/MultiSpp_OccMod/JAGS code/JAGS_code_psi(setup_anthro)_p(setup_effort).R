  ...
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psi(setup_anthro)_p(setup_effort).txt', "
      model{
        ...
        for(fo_psi in 2:8){                         # fo occupancy slopes (nfirst_order_psi)
        ...
        for(i in 1:nsites) {
          ...
          psiSpp1[i] <- betaSpp1[1]*psi_cov[i,1] + betaSpp1[2]*psi_cov[i,2] + betaSpp1[3]*psi_cov[i,3] + betaSpp1[4]*psi_cov[i,4] + betaSpp1[5]*psi_cov[i,12] + betaSpp1[6]*psi_cov[i,13] + betaSpp1[7]*psi_cov[i,14] + betaSpp1[8]*psi_cov[i,15]  
          psiSpp2[i] <- betaSpp2[1]*psi_cov[i,1] + betaSpp2[2]*psi_cov[i,2] + betaSpp2[3]*psi_cov[i,3] + betaSpp2[4]*psi_cov[i,4] + betaSpp2[5]*psi_cov[i,12] + betaSpp2[6]*psi_cov[i,13] + betaSpp2[7]*psi_cov[i,14] + betaSpp2[8]*psi_cov[i,15]  
          
          #'  ...for state Spp12
          psiSpp12[i] <- betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,3] + betaSpp12[4]*psi_inxs_cov[i,4] + betaSpp12[5]*psi_inxs_cov[i,12] + betaSpp12[6]*psi_inxs_cov[i,13] + betaSpp12[7]*psi_inxs_cov[i,14] + betaSpp12[8]*psi_inxs_cov[i,15]  
