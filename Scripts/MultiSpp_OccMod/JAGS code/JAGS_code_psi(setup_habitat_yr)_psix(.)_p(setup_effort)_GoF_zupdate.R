  ...
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psi(setup_habitat_yr)_psix(.)_p(setup_effort)_GoF_zupdate.txt', "
      model{
        ...
        for(i in 1:nsites) {
          for(j in 1:nsurveys) {
            ...
            y.sim[i,j] ~ dcat(rdm[i, j, (1:ncat), z.sim[i]]) #'  using z simulated under the model
        ...
        for(i in 1:nsites) {
          ...
          z2[i] <- z[i]
          z_A[i] <- ifelse(z2[i]==2 || z2[i]==4, 1, 0)
          z_B[i] <- ifelse(z2[i]==3 || z2[i]==4, 1, 0)
