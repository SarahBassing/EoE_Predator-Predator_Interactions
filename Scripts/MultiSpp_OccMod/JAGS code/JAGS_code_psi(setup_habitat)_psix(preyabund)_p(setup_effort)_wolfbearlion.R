  ...
  #'  Excluding moose effects on bear|no wolf (or bear|no lion) sub-model b/c 
  #'  do not expect bear/lion distribution to be influenced by moose 
  #'  ------------------------------------
  
  cat(file = './Outputs/MultiSpp_OccMod_Outputs/JAGS_output/JAGS_code_psi(setup_habitat)_psix(preyabund)_p(setup_effort)_wolfbearlion.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate order: Intercept[1] + Setup[2] + Elevation[3] + Forest[4] + Elk[7] + Moose[8] + White-tailed deer[10] 
          psiSpp1[i] <- betaSpp1[1]*psi_cov[i,1] + betaSpp1[2]*psi_cov[i,2] + betaSpp1[3]*psi_cov[i,3] + betaSpp1[4]*psi_cov[i,4] + betaSpp1[5]*psi_cov[i,7] + betaSpp1[6]*psi_cov[i,8] + betaSpp1[7]*psi_cov[i,10] 
          psiSpp2[i] <- betaSpp2[1]*psi_cov[i,1] + betaSpp2[2]*psi_cov[i,2] + betaSpp2[3]*psi_cov[i,3] + betaSpp2[4]*psi_cov[i,4] + betaSpp2[5]*psi_cov[i,7] + betaSpp2[6]*psi_cov[i,10] 
